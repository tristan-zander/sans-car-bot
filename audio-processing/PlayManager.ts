...
export class PlayManager {
  ...
  constructor() { this.activeMusicPlayers = {}; }
    ...
    if (message.member.voice.channel) {
      ...
      async function getStream() {
        ...
        if (url.includes('youtu')) {
          const str = await ytdl(url).then((stream) => { return stream; });
          return str;
        } else {
          return url;
        }
      ...
      this.activeMusicPlayers[message.guild.id]
          .play(await getStream(), message.member.voice.channel)
